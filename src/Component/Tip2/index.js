...
export default class TipComponent extends React.Component {
    ...
    render() {
        ...
        const tmp = selected.filter(data => data === id);   
        const isSelected = tmp.length === 1 ? true : false;
        
        return(
            <TouchableOpacity style={[styles.tipPane, isSelected ? {borderColor: '#007A78', borderWidth: 2} : {}]} onPress={() => this.touch()}>
                ...
                <TouchableOpacity style={{width: 31, height: 31, borderWidth: 2, borderColor: 'white', borderRadius: 30, justifyContent: 'center', alignItems: 'center'}} onPress={() => this.touch()}>
                    {
                        isSelected ? <View style={{width: 20, height: 20, backgroundColor: '#007A78', borderRadius: 20}}></View> : null
                    }
                </TouchableOpacity>                
            </TouchableOpacity>
        );
